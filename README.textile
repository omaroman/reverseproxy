...
** type = SchemeType.HTTP or SchemeType.HTTPS, for using either HTTP or HTTPS scheme
...
** type = SchemeType.HTTP (default value) or SchemeType.HTTPS, for using either HTTP or HTTPS scheme

With GlobalSwitchScheme all Methods from a Controller will use the specified or default SchemeType value, unless
a method(s) are annotated with SwitchScheme, in which case, that method will override the global SchemeType value.
...
when the app validates that the user is authentic, afterwards the app shall redirect to the restrictedMethod route (REFERRED_URL).
